...
<div class="page-wrapper">
  ...
  <div class="row page-titles">
    <div class="col-md-5 align-self-center">
      <h5 class="text-themecolor"><i class="fa fa-archive" aria-hidden="true"></i> K8 Withdrawal Order Report</h>
    </div>

    <div class="col-md-7 align-self-center">
      <ol class="breadcrumb">
        <li class="breadcrumb-item"><a href="javascript:void(0)">Home</a></li>
        <li class="breadcrumb-item active">K8 Withdrawal Order Report</li>
      </ol>
    </div>
  </div>
    ...
    <div class="modal fade" id="orderModal" tabindex="-1" role="dialog" aria-labelledby="orderModalLabel">
      <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
          ...
          <form method="post" action="<?= base_url('K8_W/Save_K8_W') ?>" id="orderForm">
    ...
    <div class="container-fluid">
      ...
      <div class="grid-container" style="margin-top: 20px;">
        <div class="card" style="--grad: #FFC107, #FF9800;">
          <center>
            <div class="title">
              <i class="fas fa-users" style="color: #FF9800;"></i> K8 Withdrawal Project Employees 
            </div>
          </center>
          <br>
          <br>
          <div class="content">
            <center>
              <h2>
                ...
                $this->db->where('project', 'K8 Withdrawal');
...
.card:hover {
            ...
            <tbody>
              <?php foreach ($k8_w as $order): ?>
                ...
                <tr>
                  ...
                  <td>
                    <button class="btn btn-outline-success btn-sm me-1" onclick='editOrder(<?= json_encode($order) ?>)'>
                      <i class="bi bi-pencil-square"></i> </button>


                    <a href="<?= base_url("K8_W/Delete_K8_W/{$order->order_id}") ?>"
  ...
  <script>
    function editOrder(order) {
      ...
      $('#orderForm').attr('action', '<?= base_url("k8_W/Update_K8_W") ?>');
  ...
  <script>
    function fetchChartData() {
      fetch(`<?= base_url('K8_W/get_all_orders_barline_chart') ?>`)
        .then(res => res.json())
        .then(data => {
  ...
  let url = `<?= base_url('K8_W/get_all_orders_barline_chart') ?>?${params.toString()}`;
  ...
  let totalUrl = `<?= base_url('k8_W/get_filtered_order_sum') ?>?${params.toString()}`;
  ...
  if (reportLabel) {
    ...
    reportLabel.textContent = `Order Duration for K8 Withdrawal Project -- ${startDate} to ${endDate} (${diffDays} day${diffDays > 1 ? 's' : ''})`;
  }
}
 ...
 <script>
  function resetAndReload() {
    ...
    fetch(`<?= base_url('K8_W/get_filtered_order_sum') ?>`)
      .then(res => res.json())
      .then(data => {
        document.getElementById("totalOrderCount").textContent = data.total ?? 0;
      })
      ...
      $.ajax({
        ...
        success: function (response) {
          ...
          if (res.status === 'success') {
            ...
            if (actionUrl.includes('Save_k8_W')) {
             
              $('#orderForm')[0].reset();
            } else if (actionUrl.includes('Update_K8_W')) {
