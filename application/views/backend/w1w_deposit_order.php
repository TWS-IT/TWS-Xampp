2<?php $this->load->view('backend/header'); ?>
...
.card:hover {
  ...
  <script>
    function editOrder(order) {
      ...
      $('#orderForm').attr('action', '<?= base_url("W1W_Deposite_Order/Update_W1W_D") ?>');
  ...
  <script>
    function fetchChartData() {
      fetch(`<?= base_url('W1W_Deposite_Order/get_all_orders_barline_chart') ?>`)
        .then(res => res.json())
        .then(data => {
  ...
  if (reportLabel) {
    ...
    reportLabel.textContent = `Order Duration for W1W_Deposit Project -- ${startDate} to ${endDate} (${diffDays} day${diffDays > 1 ? 's' : ''})`;
  }
}
