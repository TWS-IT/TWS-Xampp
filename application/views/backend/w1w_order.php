...
<?php } ?>
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      <form method="post" action="<?= base_url('W1W_Order/Save_W') ?>" id="orderForm">
...
<body>
  ...
  <div class="row">
    ...
    <tbody>
       <?php foreach ($w1w_order as $order): ?>
    <tr>
        ...
        <td>
         <button class="btn btn-outline-success btn-sm me-1"onclick='editOrder(<?= json_encode($order) ?>)'> <i class="bi bi-pencil-square"></i> </button>
            ...
            <a href="<?= base_url("W1W_Order/Delete_W/{$order->order_id}") ?>" onclick="return confirm('Are you sure you want to delete this order?')" class="btn btn-outline-danger btn-sm"> <i class="bi bi-trash"></i> </a>
...
<!-- <script>
  document.addEventListener("DOMContentLoaded", function () {
    const selectBox = document.querySelector('#employee-select');
    if (selectBox) {
      dselect(selectBox, { search: true });
    }
  });
</script> -->
...
<script>
  function editOrder(order) {
    ...
    $('#orderForm').attr('action', '<?= base_url("W1W_Order/Update_W") ?>');
...
$(document).ready(function () {
  $.ajax({
    url: "<?= base_url('W1W_Order/get_all_orders_barline_chart') ?>",
    method: "GET",
    success: function (response) {
      const data = JSON.parse(response);

      // Sort total_orders and avg_orders arrays by date (x)
...
function filterChart() {
  ...
  fetch(`<?= base_url('W1W_Order/get_all_orders_barline_chart') ?>?date_from=${startDate}&date_to=${endDate}`)
    .then(res => res.json())
    .then(data => {
      if (chart) {
...
function fetchChartData() {
  fetch(`<?= base_url('W1W_Order/get_all_orders_barline_chart') ?>`)
    .then(res => res.json())
    .then(data => {
      if (chart) {
        chart.updateOptions(getChartOptions(data));
...
function filterChart() {
  ...
  fetch(`<?= base_url('W1W_Order/get_all_orders_barline_chart') ?>?${params.toString()}`)
    .then(res => res.json())
    .then(data => {
