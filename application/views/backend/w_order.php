          ...
          <div class="content">
            <center>
              <h2>
                ...
                $this->db->where('project', 'W Project');
          ...
          <div class="content">
            <center>
              <h2>
                ...
                $this->db->where("e.project", "W Project");
                echo $this->db->count_all_results();
                ?>
              </h2>
                    ...
                    <tr>
                      ...
                      <td>
                        ...
                        <?php if (in_array($this->session->userdata('user_type'), ['SUPER ADMIN', 'ADMIN'])): ?>
                          ...
                          <a href="<?= base_url("W_Order/Delete_W/{$order->order_id}") ?>"
...
fetch(`<?= base_url('W_Order/showMistakeChart') ?>/${encodeURIComponent(project)}`)
    .then(res => res.json())
    .then(data => {
      ...
      const maxMistakes = Math.max(...Object.values(dailyCounts));
      ...
      const options = {
        ...
        fill: {
          type: 'gradient',
          gradient: { shadeIntensity: 0.5, opacityFrom: 0.6, opacityTo: 0 }
        },
        colors: ['#FF0000']
      };
    ...
    <script>
      function fetchChartData() {
        let url = `<?= base_url('W_Order/get_all_orders_barline_chart') ?>?shiftid=ALL`;
