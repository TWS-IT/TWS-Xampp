...
public function emp_perfomance($em_code = null)
{
    ...
    // Combine orders from all project tables
    $project_tables = ['w_order','atas_order','w1w_deposit_order','w1w_w','k8_d','k8_w'];
    foreach ($project_tables as $table) {
        if ($this->db->table_exists($table)) {
            $this->db->select('order_date, SUM(order_count) as orders');
            $this->db->where('employee_id', $employee->em_code);
            $this->db->group_by('order_date');
            $query = $this->db->get($table);

            foreach ($query->result() as $row) {
                $total_orders += $row->orders;
                $order_dates[$row->order_date] = true;
            }
        }
    }
    ...
    // Total mistakes
    $total_mistakes = $this->db
        ->where('emp_id', $employee->em_code)
    ...
    // Performance ratio
    $raw_efficiency = ($total_days > 0) ? ($total_orders / ($daily_target * $total_days)) * 100 : 0;

    // Apply penalties properly
