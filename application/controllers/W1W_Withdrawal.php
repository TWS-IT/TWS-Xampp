...
class W1W_Withdrawal extends CI_Controller {
    ...
    public function Get_Data(){
        ...
        $data['w1w_withdrawal'] = $this->w1w_w_model->Get_w();
    }
...
$this->form_validation->set_rules('order_count', 'order_count', 'required');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $this->w1w_w_model->Add_w($data);  // Make sure your model handles this
            $this->session->set_flashdata('feedback', 'Successfully Added');
            log_action($this, 'Save', "Order for employee '{$data['employee_id']}' added.");
            redirect('W1W_Withdrawal/w1w_withdrawal'); 
        }
    } else {
    ...
    function w1w_withdrawal()
	{
		if ($this->session->userdata('user_login_access') != False) {
			...
			$this->load->view('backend/w1w_withdrawal', $data);
		} else {
			redirect(base_url(), 'refresh');
		}
	}
    ...
    if ($this->session->userdata('user_login_access') != False) {
        ...
        $this->w1w_w_model->update_W($id, $data);
...
public function Edit_W($id) {
    if ($this->session->userdata('user_login_access') != False) {
        ...
        $this->load->view('backend/edit_w1w_withdrawal', $data); 
    } else {
        redirect(base_url(), 'refresh');
    }
}
...
public function Delete_W($id) {
    if ($this->session->userdata('user_login_access') != False) {
        $this->w1w_w_model->DeleteWOrder($id);
...
//     $data = $this->w1w_w_model->get_filtered_barline_chart_se($start_date, $end_date, $employee_id);
...
//         $data = $this->w1w_w_model->get_all_orders_for_barline_chart($startDate, $endDate, $employee_id);
//         echo json_encode($data);
//     } else {
//         show_error("Unauthorized access", 403);
//     }
